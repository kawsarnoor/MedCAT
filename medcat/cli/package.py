...
def verify_model_package(request_url, headers, full_model_tag_name, git_auth_token):
    ...
    if not found_matching_folders and available_model_tags:
...
def select_model_package_and_name(model_name, previous_model_tag_data=False):
    """
    """
    ...
    if model_name != "":
        ...
        if previous_model_tag_data != False and model_name == previous_model_tag_data.model_name:
            if prompt_statement("The model you want to package is based on the following model:" + "\033[1m" + model_name + "-" + previous_model_tag_data.version + "\033[0m" + " , is this correct? Choosing NO means this will be treated as a new release."):
                version_choice = model_name + "-" + previous_model_tag_data.version
            else:
                version_choice = input("Give the model tag a name, the version will be 1.0 by default:")
                model_name = version_choice 
        ...
        #if model_name not in available_model_tags:
        #    logging.warning("Model name found in the repository containing the name : " + model_name )
        ##else:
#
#
        #if model_name not in found_matching_folders:
        #    logging.warning("Model name found locally containing the name : " + model_name )
        #else:
        
        if version_choice != "":
            return os.path.join(get_local_model_storage_path(), version_choice), model_name, is_new_release
        
    elif model_name == "" and previous_model_tag_data is False:
        logging.error("No model name has been provided, and the models detected in the current folder have no model tag data history.... exiting")
        sys.exit()
...
def copy_model_files_to_folder(source_folder, dest_folder):
    ...
    for file_name in files:
        if file_name in get_permitted_push_file_list():
            ...
            print("Discarding " + file_name + " as it is not in the permitted model file pushing convention...")
    print("\n")

def inject_tag_data_to_model_files(mrequest_url, headers, model_folder_path, model_name, parent_model_name, version, commit_hash, git_repo_url):
...
def upload_model(model_name, parent_model_name, version, git_auth_token, git_repo_url):
    ...
    model_package_folder, selected_model_name, is_new_release = select_model_package_and_name(model_name, previous_tag_model_data)
    model_name = selected_model_name

    print(model_package_folder, selected_model_name, is_new_release)

    if is_new_release:
        if not os.path.exists(model_package_folder):
            os.makedirs(model_package_folder)
        else:
            shutil.rmtree(model_package_folder, ignore_errors=True)
            os.makedirs(model_package_folder)
        if not is_dir_git_repository(model_package_folder):
            version = "1.0"
            create_new_base_repository(model_package_folder, git_repo_url)
    
    #if is_new_release:
    #    if model_package_folder != "":
    #        if not os.path.exists(model_package_folder):
    #        os.makedirs(model_package_folder)
    #        if not is_dir_git_repository(model_package_folder):
    #        version = "1.0"
    ...
    copy_model_files_to_folder(original_folder, model_package_folder)
    ...
    try:
        ...
        print("Current working dir: ", model_package_folder)
        print("===================================================================")
        print("Git status:")
        subprocess.run(["git", "status"], cwd=model_package_folder)
        print("===================================================================")
        print("DVC status:")
        subprocess.run([sys.executable, "-m", "dvc","status"], cwd=model_package_folder)
        print("===================================================================")

        repo = Repo(model_package_folder, search_parent_directories=False)
        ...
        subprocess.run([sys.executable, "-m", "dvc", "commit"], cwd=model_package_folder) 
        ...
        if untracked_files or changed_files:
                    ...
                    for file_name in untracked_files:
                        if prompt_statement("Add : " + file_name + " to the DVC repo ?"):      
                            if ".dvc" not in file_name and file_name not in repo.ignored(file_name):
                                subprocess.run([sys.executable, "-m", "dvc","add", file_name], cwd=model_package_folder)
            ...
            for root, dirs, files in os.walk(model_package_folder):
        ...
        if staged_files:
            ...
            if prompt_statement("Do you want to create the tag: " + tag_name + " and release " + release_name + "?" ):
                ...
                tag_data = {
                    ...
                    "body" : generate_model_card_info(repo, model_name, model_package_folder, parent_model_name, version, tag_name)
                }
                ...
                if create_tag_request.status_code == 201:
                    ...
                    subprocess.run(["git", "bundle", "create", str(tag_name) + ".bundle", "--all"], cwd=model_package_folder)
                    bundle_file_path = model_package_folder + "/" + str(tag_name) + ".bundle"
                        
                    #if "nt" in os.name:
                    #    subprocess.call(["tar.exe", "-af", str(tag_name) + ".zip", bundle_file_path ], cwd=model_package_folder)
                    #else:
                    #    subprocess.call(["zip", "-9 -y -q", str(tag_name) + ".zip", bundle_file_path ], cwd=model_package_folder)
                    #subprocess.call(["tar", "-zf", str(tag_name) + ".tar.gz", bundle_file_path ], cwd=model_package_folder)
                ...
                subprocess.call([sys.executable, "-m", "dvc", "push"], cwd=model_package_folder)
            ...
            subprocess.run([sys.executable, "-m", "dvc", "push"], cwd=model_package_folder)
        ...
        subprocess.run(["git", "reset", "HEAD~"], cwd=model_package_folder)
        if tag_name != "": 
            print("Deleting tag ", tag_name)
            #subprocess.run(["git", "push", "origin", "--delete", tag_name ], cwd=model_package_folder)
            subprocess.run(["git", "tag", "--delete", tag_name ], cwd=model_package_folder)
